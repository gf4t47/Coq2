...
Proof.
  intros.
  destruct H as [[HLP | HLQ] [HRP | HRR]].
  Case "left".
    left. apply HLP.
    left. apply HLP.
  Case "right".
    left. apply HRP.
    right. split. apply HLQ. apply HRR.
  Qed.
(** [] *)
...
Proof.
  ...
  Case "left".
    ...
    split.
    SCase "S left".
      inversion H. left. apply H0.
      inversion H0. right. apply H1.
    SCase "S right".
      inversion H. left. apply H0.
...
Proof.
  ...
  Case "b = false". right. simpl in H. apply H.
  Qed.
...
Proof. 
  intros.
  destruct b.
  Case "b = true". simpl in H. inversion H.
  Case "b = false". simpl in H.
...
Proof.
  intros P.
  intro excluded_middle_False.
  apply excluded_middle_False.
  right. intro. 
  apply excluded_middle_False. left. apply H.
  Qed.
...
Proof.
  ...
  Case "n <> 0".
    ...
    destruct m.
    SCase "m = 0". simpl in H. inversion H.
    SCase "m <> 0". simpl in H. apply IHn' in H. rewrite -> H. reflexivity.
  Qed.
...
Proof. 
  intros.
  unfold not in H.
  remember (beq_nat n m) as beq_nm.
  destruct (beq_nm).
  Case "n = m".
    apply ex_falso_quodlibet.
    apply H.
...
Theorem beq_ref_true: forall n : nat,
  beq_nat n n = true.
Proof.
  induction n as [|n'].
  reflexivity.
...
Proof.
  ...
  Case "n = 0".
    ...
    SCase "m <> 0". unfold not. intros. inversion H0.
  Case "n = S n'".
    intros.
    destruct m.
    SCase "m = 0". unfold not. intros. inversion H0.
    SCase "m <> 0". unfold not. intros. rewrite H0 in H. simpl in H. rewrite beq_ref_true in H. inversion H.
Qed.
(** [] *)
